...
def render_quan_ly_loi(current_menu_name, current_user_role=None, user_perms=None):
    ...
    is_admin = (current_user_role == "Admin" or (user_perms and user_perms.get("perm_input", False)))
    ...
    if not raw_input_df.empty:
        ...
        if not paginated_df.empty:
            can_delete_data = (current_user_role == "Admin" or (user_perms and user_perms.get("perm_input", False)))
                                        ...
                                        with sub_cols[i]:
                                            try:
                                                if u.startswith("http://") or u.startswith("https://") or u.startswith("data:image"):
                                                    # Sử dụng trực tiếp st.image với width nhỏ gọn, tích hợp sẵn nút Fullscreen chuẩn Streamlit khi rê chuột
                                                    st.image(u, width=60)
